...
- Página de compartir (`/compartir`): social sharing con botones de Facebook, X/Twitter, LinkedIn y WhatsApp, copiar enlace, Web Share API nativa (`navigator.share`) y vista previa de OG card. Acepta tres combinaciones de query params:
  - `?date=YYYY-MM-DD`: lectura del día litúrgico. Cada sección (Primera, Salmo, Segunda, Evangelio) muestra su referencia con un enlace "Compartir" que navega a la vista de cita (`?date=X&ref=Y&version=Z`).
  - `?ref=<cita>&version=<code>` (o `?date=X&ref=Y`): vista de cita única. Resuelve la referencia con `parseBibleReferenceQuery`, carga los versículos server-side vía `readChapterRpc` (incluye citas entre capítulos como `1 Co 12,31-13,13`, filtradas por `filterVersesByReference`) y genera las URLs de share hacia el deep link `/?modo=busqueda&q=<cita>&version=<code>`.
  - Consultas no referenciales (palabras clave): la vista muestra la consulta sin versículos y el enlace compartido abre el modo búsqueda con esa `q`. La `version` se valida contra el catálogo (`getEnabledBibleVersion`) con fallback a la versión por defecto.
  - Meta tags OG dinámicos: título = cita normalizada, descripción = primer tramo del texto; imagen OG compartida (`og-biblia.png`).
...
- ShareButton en modales y paneles (`PortalBibleHub`): modo Lectura incluye compartir en la fila de acciones del panel y en el footer del modal de capítulo (`?ref=<Libro capitulo>`); modo Búsqueda comparte la referencia normalizada en búsquedas por referencia y cada resultado individual en búsquedas por palabra clave (`?ref=<cita del resultado>`); modo Liturgia envía la cita específica del slot más la fecha (`?ref=<cita>&date=<fecha>`); todos incluyen `version=<code>` activo para preservar contexto.
