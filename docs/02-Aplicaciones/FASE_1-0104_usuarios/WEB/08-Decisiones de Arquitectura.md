...
| ADR-0104-USUARIOS-006 | Las consultas protegidas usan clientes Supabase ligados al JWT de la request y la asignación de roles se ejecuta mediante RPC transaccional. | Evita que un cliente anónimo consulte/escriba datos como si estuviera autenticado y evita cambios de rol sin historial/auditoría completa.                                         | Las apps consumidoras deben pasar el token al UserService y desplegar la migración RBAC.                            |
